...
### Added
- **`erp-report-engine mcp --demo` — a zero-setup guarded MCP server.** Serves the six tools over the bundled demo database (built in a temp dir), so anyone can start the server and introspect/query it with no ERP and no config — `mcp --demo`, or `docker run` (now the image's default command). The real path stays `mcp -c config.yaml`.
- **Installable as a Claude Code plugin.** `/plugin marketplace add gulmezeren2-byte/erp-report-engine` then `/plugin install erp-report-engine` bundles the guarded MCP server (launched via `uvx`, no manual install), the safe-query skills, and zero-setup `/trust-benchmark` and `/erp-schema` commands. `.claude-plugin/{plugin,marketplace}.json` wire it; the skills and commands work before a database is configured.
...
- **The benchmark now measures the contrast, not just our own pass.** The same corpus is run through the naive checks real tools actually ship — a starts-with-`SELECT` head check and a write-keyword blocklist (`baseline_guards.py`) — and the results page, the CLI, and the JSON all show it: the shape check refuses 6 of 28 attacks, the keyword scan 9 of 28 (and it *blocks a legitimate read* whose string literal contains the word "delete"), while this guard refuses all 28 and blocks none. A test pins that our guard strictly dominates every baseline, so "we're better" is a measured fact, not a slogan.
- **The guard is now fuzzed, not only spot-checked.** Hypothesis property tests (`tests/test_guard_fuzz.py`) dress every denylisted function up thousands of ways — arbitrary case, whitespace before the parenthesis, varied arguments, across four dialects — and assert it is still refused, and that writes are refused for any object name; in the other direction, a plain read with arbitrary (safe) identifiers still passes, so the guard is selective, not a wall. The dangerous-function list is imported from the guard, so a newly denylisted name is fuzzed automatically.
- **An in-browser guard playground** ([playground.html](https://gulmezeren2-byte.github.io/erp-report-engine/playground.html)) that runs the *actual* `guard.py` in the visitor's browser via Pyodide — paste any SQL and watch it allow or refuse it, client-side, nothing sent anywhere. It embeds the real guard source (CI fails on drift), so it is the code the tests run, not a re-implementation.
- **A security case-study** and a **fair comparison** of read-only DB-access approaches (transactions, roles, statement guards, semantic layers, gateways — honest about where each wins), plus a real **landing page** hub. All on the same self-contained dark palette.
- **`describe_model` is now a real semantic layer**, not a list of column names: every entity's grain, every column's type and *meaning* (including gotchas an agent can't infer, like `actual_ship_date` being NULL until shipment), and runnable example queries per entity — with a test that every advertised example actually executes through the guarded path.
- **Automatic listing on the official MCP registry on release** — a `server.json` and a `publish-mcp` workflow job that authenticates via GitHub OIDC (no stored token) after the PyPI upload, best-effort so a preview-registry hiccup can't fail a release.

### Changed
- **Repositioned to lead with the read-only agent/MCP layer**; weekly reports become the proof surface, not the sole identity. README, package description, keywords, and the landing page follow. Nothing about the engine changed — positioning caught up to what the tool already is.
- **The read-only guard moved to its own dependency-light module** (`erp_report_engine/guard.py`, only `re` + `sqlglot`), so it runs standalone — which is what lets the browser playground load the genuine code. `connect.py` re-exports it (every import unchanged); `ReadOnlyViolation` now lives in `errors.py`.
- **On-time % now uses a p-chart, not XmR.** A proportion over a varying denominator needs limits that widen when the week is thin — an individuals chart weighted a 2-delivery week like a 200-delivery one (and drew an impossible 101.5% upper limit). Revenue keeps XmR. Both quote one receipt string, built once and carried to the report, the dashboard, and Power BI (`meta_spc.csv` now ships the method + a ready receipt instead of Power BI reconstructing an XmR formula that would be wrong for the proportion).
- **The Power BI aging chart is coloured by severity** (green → red) via a `Bucket Colour` measure, instead of painting every bucket one colour.
- The attack corpus widened to 28 (the two famous incidents + more SQL Server / MySQL vectors — `xp_cmdshell`, `sp_OACreate`, `INTO OUTFILE`, `COPY ... TO`, comment-obfuscation).
- Docker base image pinned by digest; CONTRIBUTING and the issue/PR templates lead with guard-bypass reports and document the actual CI gates; ARCHITECTURE and AGENTS brought current to the four-layer guard.
