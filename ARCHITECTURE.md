...
`pytest tests/` covers the guard (the shared attack corpus, hostile cases, per dialect; **fuzzed** with hypothesis so a denylisted function is refused however it is cased, spaced, or argumented, and a plain read with arbitrary identifiers still passes; and **measured against naive baselines** — a starts-with-`SELECT` check and a keyword blocklist — so "it strictly beats the shortcuts real tools ship" is a tested fact, not a claim), profile contracts, the calendar core (unit + hypothesis property tests), render escaping, the honesty fixes, CLI exit codes, the MCP tools (including that the semantic layer's example queries actually run and that `query` can't escape the canonical entities), the SPC methods, and a full end-to-end run plus PBIP integrity on the bundled demo database. CI additionally runs `ruff`, a coverage floor, and fails on drift of the generated PBIR pages and the generated docs pages (`trust.html` / `playground.html`), on Windows and Linux across Python 3.10–3.13.
