...
| `MetaSpc` table | **Control limits as data** | The XmR limits the engine computed, exported rather than reimplemented in DAX — a second implementation is eventually a second answer |
...
4. **Aging** *(when receivables are mapped)* — total AR, overdue %, and 91+ cards, the open balance by age bucket, and overdue ranked by customer: chase the oldest money first.
